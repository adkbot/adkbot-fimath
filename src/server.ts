    ...
    lines.forEach((line: string) => {
      if (line.trim()) {
        try {
          ...
          console.log(`[MT5 BRIDGE] ${JSON.stringify(json)}`);
...
app.get('/api/connect-mt5', (req, res) => {
  ...
  // If already running, don't kill it. Just notify the new client.
  if (pythonProcess) {
    res.write(`data: ${JSON.stringify({ type: 'log', message: 'Bridge já está ativa e operando.' })}\n\n`);
    // Also send an immediate connection_success so the UI knows we are good
    res.write(`data: ${JSON.stringify({ type: 'connection_success', message: 'Bridge re-sincronizada.' })}\n\n`);
  } else {
    res.write(`data: ${JSON.stringify({ type: 'log', message: 'Iniciando bridge de conexão...' })}\n\n`);
    startPythonProcess();
  }
...
app.get('/api/calendar', async (req, res) => {
    try {
        // Fetch from ForexFactory (Reliable free source) - Add User-Agent to avoid blocks
        ...
        try {
            ...
            // Filter for today's events or recent ones
            const now = new Date();
            const todayStr = now.toISOString().split('T')[0];
            
            const todayEvents = data.filter((e: any) => {
                if (!e.date) return false;
                // Accept events from today or where date matches YYYY-MM-DD
                return e.date.includes(todayStr) || e.date === todayStr;
            });
            
            res.json(todayEvents);
        } catch (e) {
            console.error("Calendar JSON Parse Error:", text.substring(0, 100));
            res.json([]);
        }
    } catch (error) {
